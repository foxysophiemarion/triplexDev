...
class FullPage {
	constructor(element, options) {
		let config = {
			...
			// Класс для врапера во время переключения
	...
	setClasses() {
		// Сохранение id для ПРЕДЫДУЩЕЙ секции (если такая есть)
		this.previousSectionId = (this.activeSectionId - 1) >= 0 ?
			this.activeSectionId - 1 : false;

		// Сохранение id для СЛЕДУЮЩЕЙ секции (если такая есть)
		this.nextSectionId = (this.activeSectionId + 1) < this.sections.length ?
			this.activeSectionId + 1 : false;

		// Установка класса и присвоение элемента для АКТИВНОЙ секции
		...
		// Установка класса и присвоение элемента для ПРЕДЫДУЩЕЙ секции
		...
		// Установка класса и присвоение элемента для СЛЕДУЮЩЕЙ секции
	...
	switchingSection(idSection = this.activeSectionId, direction) {
		...
		this.stopEvent = true;
		// Если секция крайняя, то разрешаем события
